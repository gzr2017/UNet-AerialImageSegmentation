...
def train(unet, i_net, train_dataset, test_dataset=None):
    ...
    norm_gradients_node = tf.Variable(tf.constant(0.0, shape=[len(unet.gradients_node)]), name='norm_gradients')
    with tf.Session() as sess:
        ...
        for epoch in range(EPOCHS):
            ...
            for step in range(epoch * train_dataset.iterations, (epoch + 1) * train_dataset.iterations):
                ...
                _, loss, gradients = sess.run((optimizer, unet.cost, unet.gradients_node), feed_dict={
                    unet.x: train_x,
                    unet.y: train_y,
                    unet.keep_prob: KEEP_PROB
                })
                if NORM_GRADS:
                    avg_gradients = update_avg_gradients(avg_gradients, gradients, step)
