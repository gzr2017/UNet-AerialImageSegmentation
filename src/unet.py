...
def output_class(prediction):
    ...
    for h in range(batch_size):
        for i in range(rows):
            for j in range(cols):
                classed_prediction[h, i, j, most_possible_position[h, i, j]] = 1
    return classed_prediction


def class_to_color(class_prediction, save_path, save_name):
    reverse_COLOR_CLASS_DICT = dict(
        zip(COLOR_CLASS_DICT.values(), COLOR_CLASS_DICT.keys()))
    prediction = np.reshape(class_prediction,
                            newshape=[class_prediction.shape[1], class_prediction.shape[2],
                                      class_prediction.shape[3]])
    [rows, cols, _] = prediction.shape
    color_value = np.zeros((rows, cols, IMG_CHANNEL), dtype=np.uint8)
    for i in range(rows):
        for j in range(cols):
            for k in range(N_CLASS):
                if prediction[i][j][k] == 1:
    ...
    real_out_predict_image.save(path.join(save_path, save_name + '.tif'))


class UNet(object):
    def __init__(self, cost_kwargs={}, **kwargs):
        self.x = tf.placeholder(tf.float32, shape=[None, None, None, IMG_CHANNEL], name='x')
        self.y = tf.placeholder(tf.float32, shape=[None, None, None, N_CLASS], name='y')
        self.keep_prob = tf.placeholder(tf.float32, name='keep_prob')
        self.output_map, self.variables = build_unet(self.x, self.keep_prob)
        ...
        with tf.name_scope('results'):
            ...
            self.correct_pred = tf.equal(tf.argmax(self.output_map, 3), tf.argmax(self.y, 3))
            # 准确率
    ...
    def predict(self, image, model_path, save_path):
        ...
        test_x = np.reshape(test_x, newshape=[1, test_x.shape[0], test_x.shape[1], -1])
