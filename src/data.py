...
def color_to_class(img_path, save_path):
    ...
    for aerial_image_label in aerial_image_labels:
        ...
        for i in range(rows):
            for j in range(cols):
                label_value[i, j, COLOR_CLASS_DICT[tuple(raw_aerial_image_label[i, j])]] = 1
        np.save(path.join(save_path, path.basename(aerial_image_label).split('.')[-2]), label_value)
...
def create_dataset(image_data_source, label_data_source, tfrecord_save_path):
    ...
    tfrecord_name_generator = file_name_generator(ex_name='tfrecord', file_name='aerial_pair')
...
def parse_dataset(proto):
    dics = {
        'aerial_image_label':
            tf.FixedLenFeature(shape=[OUTPUT_IMG_SIZE, OUTPUT_IMG_SIZE, N_CLASS], dtype=tf.float32),
        'aerial_image':
            tf.FixedLenFeature(shape=[IMG_SIZE, IMG_SIZE, IMG_CHANNEL], dtype=tf.float32)
    }
...
class DatasetDir(object):
    def __init__(self, base_dir):
        self.dir_dict = {
            ...
            'split_label_classed': path.join(base_dir, 'split/label_classed'),  # 将color转为class之后的label
    ...
    def data_nagare(self, is_split=True, is_tran_class=True, is_create_dataset=True):
        if is_split and len(glob(path.join(self.dir_dict['split_data'], '*.*'))) == 0:
            split_img(self.dir_dict['original_data'], self.dir_dict['split_data'])
        if is_split and len(glob(path.join(self.dir_dict['split_label'], '*.*'))) == 0:
            split_img(self.dir_dict['original_label'], self.dir_dict['split_label'], OUTPUT_IMG_SIZE)
        if is_tran_class and len(glob(path.join(self.dir_dict['split_label_classed'], '*.*'))) == 0:
            color_to_class(self.dir_dict['split_label'], self.dir_dict['split_label_classed'])
        if is_create_dataset and len(glob(path.join(self.dir_dict['tfrecord'], '*.*'))) == 0:
            create_dataset(self.dir_dict['split_data'], self.dir_dict['split_label_classed'], self.dir_dict['tfrecord'])

    def get_a_iterator(self):
        label_num = len(glob(path.join(self.dir_dict['split_label_classed'], '*.*')))
...
class NetDir(object):
    def __init__(self, base_dir, net_name):
        ...
        for dir in self.dir_dict.values():
            if not path.exists(dir):
                makedirs(dir)
        self.net_name_generator = file_name_generator(file_name=net_name, ex_name='ckpt')