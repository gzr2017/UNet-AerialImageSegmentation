...
# SUMMARIES = True
# # 输入像的大小和通道数
# IMG_SIZE = 500  # 图像尺寸不能设置得太小了，要不然池化池化池化得没有了……
# IMG_CHANNEL = 3
# OUTPUT_IMG_SIZE = 308  # 手工计算……我也想不到什么可以自动计算的方法惹
# N_CLASS = 2
...
RESTORE = None  # 并没有名为 unet.ckpt 的实体文件。它是为检查点创建的文件名的前缀。用户仅与前缀（而非检查点实体文件）互动。
SUMMARIES = True
# 输入像的大小和通道数
...
EPOCHS = 20
OPTIMIZER = 'adam'  # 选择
HOW_TO_CAL_COST = 'bin_cross_entropy'  # 选择损失函数计算方法
MOMENTUM = 0.2
LEARNING_RATE = 1e-5
DECAY_RATE = 0.95
DISPLAY_STEP = 5  # number of steps till outputting stats
